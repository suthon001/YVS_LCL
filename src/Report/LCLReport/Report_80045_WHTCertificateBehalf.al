/// <summary>
/// Report YVS WHT Certificate (ID 80045).
/// </summary>
report 80045 "YVS WHT Certificate Behalf"
{
    DefaultLayout = RDLC;
    RDLCLayout = './LayoutReport/LCLReport/Report_80045_WHTCertificateBehalf.rdl';
    ...
    dataset
    {
        dataitem("WHT Header"; "YVS WHT Header")
        {
            ...
            begin
                ...
                REPEAT
                    ...
                    IF COPYSTR(WHTRegID2, Currcount, 1) IN ['1', '2', '3', '4', '5', '6', '7', '8', '9', '0'] THEN
                        IF DigitCount <= 13 THEN BEGIN
                            ...
                            WHTRegBehalf[DigitCount] := COPYSTR(WHTRegID2, Currcount, 1);
                            DigitCount += 1;
                        END;
                ...
                if "On Behalf No." <> '' then begin
                    CLEAR(WHTRegBehalf);
                    CLEAR(BehalfName);
                    clear(BehalfAddress);
                    BehalfName := "On Behalf Name";
                    if "On Behalf for" = "On Behalf for"::Vendor then begin
                        Vend.get("On Behalf No.");
                        BehalfAddress := Vend.Address + ' ' + Vend."Address 2" + ' ' + Vend.City + ' ' + Vend."Post Code";
                        BehalfVatRegis := DelChr(Vend."VAT Registration No.", '=', '-');
                    end;
                    if "On Behalf for" = "On Behalf for"::Customer then begin
                        Cust.get("On Behalf No.");
                        BehalfAddress := Cust.Address + ' ' + Cust."Address 2" + ' ' + Cust.City + ' ' + Cust."Post Code";
                        BehalfVatRegis := DelChr(Cust."VAT Registration No.", '=', '-');
                    end;
                    for LoopVatBehalf := 1 to 13 do
                        WHTRegBehalf[LoopVatBehalf] := COPYSTR(BehalfVatRegis, LoopVatBehalf, 1);
                end;
        ...
        dataitem(Integer; integer)
        {
            ...
            column(BehalfName; BehalfName) { }
            column(BehalfAddress; BehalfAddress) { }
            column(WHTRegRehalf1; WHTRegBehalf[1]) { }
            column(WHTRegRehalf2; WHTRegBehalf[2]) { }
            column(WHTRegRehalf3; WHTRegBehalf[3]) { }
            column(WHTRegRehalf4; WHTRegBehalf[4]) { }
            column(WHTRegRehalf5; WHTRegBehalf[5]) { }
            column(WHTRegRehalf6; WHTRegBehalf[6]) { }
            column(WHTRegRehalf7; WHTRegBehalf[7]) { }
            column(WHTRegRehalf8; WHTRegBehalf[8]) { }
            column(WHTRegRehalf9; WHTRegBehalf[9]) { }
            column(WHTRegRehalf10; WHTRegBehalf[10]) { }
            column(WHTRegRehalf11; WHTRegBehalf[11]) { }
            column(WHTRegRehalf12; WHTRegBehalf[12]) { }
            column(WHTRegRehalf13; WHTRegBehalf[13]) { }
            column(PVNo; PVNo) { }
    ...
    var
        ...
        WHTName, BehalfName : Text;
        WHTAddress, BehalfAddress : Text;
        WHTCerti: Text[250];
        whtVendorName: Text;
